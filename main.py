...
async def makeTurn(cell, channel, member):
        ...
        if isFull(grid) == True:
          print("isfull")
          await channel.send(channel.guild.get_member_named(f"{grid.player1.name}#{grid.player1.discriminator}").mention + " " + channel.guild.get_member_named(f"{grid.player2.name}#{grid.player2.discriminator}").mention + "\nNiemand hat gewonnen. Eure Statistiken werden nicht geändert!")
        ...
        if grid.checkForWin(active):
          await channel.send(channel.guild.get_member_named(f"{grid.active.name}#{grid.active.discriminator}").mention + '\nYou win! Setze Board zurück!')
          ...
          await removePlayerFromLists(channel.guild.get_member_named(grid.player1.name + "#" + grid.player1.discriminator))
          await removePlayerFromLists(channel.guild.get_member_named(grid.player2.name + "#" + grid.player2.discriminator))
          return False
        if isFull(grid) == True:
          print("isfull")
          await channel.send(channel.guild.get_member_named(f"{grid.player1.name}#{grid.player1.discriminator}").mention + " " + channel.guild.get_member_named(f"{grid.player2.name}#{grid.player2.discriminator}").mention + "\nNiemand hat gewonnen. Eure Statistiken werden nicht geändert!")
        ...
        if grid.active == grid.player1:
          ...
          await channel.send(channel.guild.get_member_named(grid.player2.name + "#" + grid.player2.discriminator).mention + ", du bist jetzt dran!")
        else:
          grid.active = grid.player1
          await channel.send(channel.guild.get_member_named(grid.player1.name + "#" + grid.player1.discriminator).mention + ", du bist jetzt dran!")
        await sendGrid(grid, channel)
...
class DcClient(discord.Client):
  async def on_ready(self):
    ...
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name="jeglicher Zahl zwischen 1-9"))
  ...
  async def on_message(self, message):
    ...
    if message.content == 'ping':
      ...
      if not len(args) == 2:
        ...
        reac_mes = await message.channel.send(target.mention + ", du wurdest von " + message.author.mention + " herausgefordert! Benutze die Reaktionen!")
        await reac_mes.add_reaction("✅")
        await reac_mes.add_reaction("✖")

    elif message.content == prefix + "stats":
      embed = discord.Embed(title=f"Statistiken für {message.author.name}#{message.author.discriminator}", colour=discord.Colour(0xa6f008), description="Folgend die Statistiken!", timestamp=datetime.datetime.utcfromtimestamp(time.time()))    
      embed.set_footer(text="justCoding TicTacToe")
      embed.add_field(name="🏆", value="Anzahl der gewonnen Runden: " + str(getStatsFromDB("win", message.author)))
      embed.add_field(name="💩", value="Anzahl der verloreren Runden: " + str(getStatsFromDB("lose", message.author)))

      if getStatsFromDB("lose", message.author) == 0:
        embed.add_field(name="🔄", value="Die K/D des Users beträgt: " + str(getStatsFromDB("win", message.author)))
      else:
        embed.add_field(name="🔄", value="Die K/D des Users beträgt: " + str(getStatsFromDB("win", message.author) / getStatsFromDB("lose", message.author)))
  ...
  async def on_reaction_add(self, reaction, user):
    ...
    if reaction.emoji == "1️⃣":
      ...
      if "{}#{}".format(user.name, user.discriminator) in playergrids:
        ...
        grid = playergrids["{}#{}".format(opponents[user].name, opponents[user].discriminator)]
      if user == reaction.message.channel.guild.get_member_named(grid.player1.name + "#" + grid.player1.discriminator):
        addStatsToDB("win", reaction.message.channel.guild.get_member_named(grid.player2.name + "#" + grid.player2.discriminator))
        addStatsToDB("lose", user)
      else:
        addStatsToDB("win", reaction.message.channel.guild.get_member_named(grid.player1.name + "#" + grid.player1.discriminator))
        addStatsToDB("lose", user)
      await removePlayerFromLists(reaction.message.channel.guild.get_member_named(grid.player1.name + "#" + grid.player1.discriminator))
      await removePlayerFromLists(reaction.message.channel.guild.get_member_named(grid.player2.name + "#" + grid.player2.discriminator))

    elif reaction.emoji == "✅":
      m = reaction.message.content.replace(', du wurdest von', '').replace(' herausgefordert! Benutze die Reaktionen!', '')
      m_split = m.split(' ')
      m_mention = m_split[1]
      ...
      playergrids.update({
        "{}#{}".format(user.name, user.discriminator): gamestate.GameState(userplayers[user], userplayers[target], reaction.message.channel)
      })
